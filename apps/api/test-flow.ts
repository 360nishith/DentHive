...
async function main() {
  ...
  if (!tenant) {
    console.log('❌ No tenant found in DB. Cannot test.');
    return;
  }
  ...
  const journey = await prisma.$transaction(async (tx) => {
    ...
    const stagesToCreate = template.stages.map((ts, index) => ({
      ...
      name: ts.name,
      cost: Math.round(template.estimatedCost / template.stages.length),
  ...
  const nextStage = currentJourney?.stages.find(s => s.sequenceOrder === currentStage!.sequenceOrder + 1);
  ...
  updatedJourney?.stages.forEach(s => {
    console.log(`   -> Stage ${s.sequenceOrder}: ${s.name} [${s.status}] ${updatedJourney.currentStageId === s.id ? '<-- CURRENT' : ''}`);
  });
